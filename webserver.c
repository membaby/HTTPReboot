...
struct timeval calculate_timeout() {
    ...
    float imaginary_load = (float)active_connections * 1.035 + 10;
    tv.tv_sec = MAX_TIMEOUT - (active_connections / imaginary_load) * MAX_TIMEOUT;
    printf("Timeout: %ld\n", tv.tv_sec);
...
void *connection_handler(void *socket_desc){
    ...
    while (1) {
        ...
        if (activity == 0) {
            ...
            break; // timeout, close the connection
        }
...
void increment_connection_count() {
    ...
    active_connections++;
    char log_message[1024];
    sprintf(log_message, "Active Connections: %d", active_connections);
    logger(log_message);
    pthread_mutex_unlock(&lock);
}
